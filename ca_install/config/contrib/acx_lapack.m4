...
dnl
dnl @version $Id: acx_lapack.m4,v 1.1.1.1 2009/02/06 23:28:53 pteuben Exp $
dnl @author Steven G. Johnson <stevenj@alum.mit.edu>
dnl
dnl Tech-X version:
dnl    $Id: acx_lapack.m4,v 1.1.1.1 2009/02/06 23:28:53 pteuben Exp $
dnl
...
case $with_lapack in
	...
	*) save_LIBS="$LIBS"; LIBS="$acx_lapack_libs $LIBS"
	AC_MSG_CHECKING([for $dgesv in $acx_lapack_libs])
	AC_TRY_LINK_FUNC($dgesv, [acx_lapack_ok=yes])
...
if test "x$LAPACK_LIBS" != x; then
	...
	AC_MSG_CHECKING([for $dgesv in $LAPACK_LIBS])
	AC_TRY_LINK_FUNC($dgesv, [acx_lapack_ok=yes;
...
if test $acx_lapack_ok = no; then
	AC_MSG_CHECKING([for $dgesv in BLAS library])
	AC_TRY_LINK_FUNC($dgesv, [acx_lapack_ok=yes; acx_lapack_libs=""])
	AC_MSG_RESULT($acx_lapack_ok)
fi
...
case "$host" in
  *-apple-darwin*)
    acx_lapack_ok=yes
    acx_lapack_libs='-framework Accelerate'
    ;;
esac
